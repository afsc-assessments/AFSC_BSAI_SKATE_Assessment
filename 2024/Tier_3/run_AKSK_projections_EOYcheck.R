...
setwd(paste0(getwd(), "/", AYR, "/Tier_3/EOY_catch_HP_update")) 
shell('main')
...
rec_table1 <-
  read.table(paste0(getwd(),"/", AYR, '/Tier_3/EOY_catch_HP_update/percentdb.out')) %>%
...
rec_table2 <-
  read.table(paste0(getwd(), "/", AYR, '/Tier_3/EOY_catch_HP_update/alt2_proj.out'), header=TRUE) %>%
...
write_csv(rec_table, paste0(getwd(), "/", AYR, '/Tier_3/EOY_catch_HP_update/rec_table_HP2024.csv'))
