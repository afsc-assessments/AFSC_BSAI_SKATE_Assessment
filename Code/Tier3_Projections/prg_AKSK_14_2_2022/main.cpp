...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 cout<<"ABC: "<<SPR_abc<< endl<<SPR_ofl<<endl;
  ...
  for (ispp=1;ispp<=nspp;ispp++) agg_cat(tac_ind(ispp))  += Obs_Catch(1,ispp);
  cout<<R<<endl;
 ...
 for (i=1;i<=nspp;i++) Bcurrent(i) = pmature_F(i)*elem_prod(n0_F(i),wt_F(i));
 alpha = 0.05;
 alphaCI = 0.1;
...
void model_parameters::Alt4_TAC(void)
{
  ...
  // Then given new "catches" solve for Fabc for that species
     ...
     // for (ispp=1;ispp<=nspp;ispp++) Actual_Catch(ispp) = Obs_Catch(ipro,ispp); dd=   ABC_Multiplier(ispp) * Get_Catch(alt,ispp); // ABC_multiplier is from setup.dat
...
void model_parameters::Project_Pops(const int& isim, const int& i)
{
  ...
  for (ispp=1;ispp<=nspp;ispp++) // ++++++++++++++Species
  {
    // if ( ! isit_const(ispp) )
    {
      double Ftmp;
...
void model_parameters::report(const dvector& gradients)
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  cout <<"===========================================  "<<endl;
  report << "__ ";
  for (ispp=1;ispp<=nspp;ispp++) report<< spname(ispp)<<" ";
  report<<endl;
  report << "EquilAvgAgeF0  " ;
    for (ispp=1;ispp<=nspp;ispp++) report << Avg_Age_F0(ispp)<< " "; report << endl;
  report << "EquilAvgAgeFabc  " ;
    for (ispp=1;ispp<=nspp;ispp++) report << Avg_Age_Fabc(ispp)<< " "; report << endl;
  ...
  if (last_phase())
  {
    ...
    for (ispp=1;ispp<=nspp;ispp++) report<< spname(ispp)<<" "<< SB100(ispp)<<" "<<.4*SB100(ispp)<<" "<<.35*SB100(ispp)<<" "<<F40(ispp)
...
void model_parameters::write_avg_age(const adstring& Title)
{
  ...
  ofstream& prof_F= *pad_prof_F;
  means_out << "  ";
  for (ispp=1;ispp<=nspp;ispp++) means_out << spname(ispp)<< " "; means_out << endl;
  means_out << "EquilAvgAgeF0  " ;
    for (ispp=1;ispp<=nspp;ispp++) means_out << Avg_Age_F0(ispp)<< " "; means_out << endl;
  means_out << "EquilAvgAgeFabc  " ;
    for (ispp=1;ispp<=nspp;ispp++) means_out << Avg_Age_Fabc(ispp)<< " "; means_out << endl;
  means_out << "AvgAgeYr"<< styr + npro -1 << " ";
    for (ispp=1;ispp<=nspp;ispp++) means_out << Avg_Age_End(ispp)/nsims<< " "; means_out << endl;
}
...
void model_parameters::write_ABCs(const adstring& Title)
{
  ...
  means_out << "     " ; for (ispp=1;ispp<=nspp;ispp++) means_out << spname(ispp)<< " " ; means_out <<endl;
...
void model_parameters::write_sim(const adstring& Title, d3_array& Outtmp)
{
  ...
  for (ispp=1;ispp<=nspp;ispp++) for (i=1;i<=npro;i++) for (k=1;k<=nsims;k++)
